defmodule PosServerWeb.PosLayoutComponents do
  ...
  def pos_layout(assigns) do
    ...
    <main id={@id} class={@class} {@rest}>
      ...
      <nav class="sidebar-rail" aria-label="Primary navigation">
        <a class="sidebar-link" href={pos_href(@active_page)} aria-current={current_page(@active_page, :pos)} aria-label="POS"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3h18v18H3z"/><path d="M7 7h10v10H7z"/></svg></a>
        <a class="sidebar-link" href={~p"/pos/customers"} aria-current={current_page(@active_page, :customers)} aria-label="Customers"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg></a>
        <a class="sidebar-link" href={~p"/pos/invoices"} aria-current={current_page(@active_page, :invoices)} aria-label="Invoice report"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 2v20h16"/><path d="M8 6h8M8 10h8M8 14h5"/></svg></a>
        <a class="sidebar-link" href={~p"/pos/inventory"} aria-current={current_page(@active_page, :inventory)} aria-label="Inventory"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m12 3 9 5-9 5-9-5 9-5Z"/><path d="m3 12 9 5 9-5M3 16l9 5 9-5"/></svg></a>
        <a class="sidebar-link" href={~p"/pos/orders"} aria-current={current_page(@active_page, :orders)} aria-label="Purchase orders"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 2h9l3 3v17H6z"/><path d="M9 10h6M9 14h6"/></svg></a>
        <a :if={Scope.allowed?(@scope, "company.settings")} id="company-settings-nav" class="sidebar-link" href={~p"/pos/company-settings"} aria-current={current_page(@active_page, :company_settings)} aria-label="Company settings"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 2-5h14l2 5"/><path d="M3 9h18v11H3z"/><path d="M7 20v-6h4v6"/><path d="M3 9c0 2 2 3 4 3s4-1 4-3c0 2 2 3 4 3s4-1 4-3"/></svg></a>
        ...
        <details id={"#{@id}-theme-selector"} class="sidebar-menu sidebar-theme-selector" phx-hook="PosTheme"><summary class="sidebar-menu-trigger" aria-label="Choose theme">◐</summary><div class="user-menu-content sidebar-menu-content"><button class="sidebar-menu-action" type="button" phx-click={JS.dispatch("pos:set-theme", detail: %{theme: "default-light"})}>Default Light</button></div></details>
      </nav>
      <div class="status-strip" aria-label="System status">
        ...
        <button id={"#{@id}-printer-status"} class="printer-status" type="button" data-printer-status data-status="disconnected" phx-hook="PrinterStatus" aria-label="Printer disconnected" title="Printer disconnected"><svg role="img" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9V3h12v6"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><path d="M6 14h12v7H6z"/></svg></button>
  ...
  def invoice_details_skeleton(assigns) do
    ~H"""
    <div class="card-content invoice-details-skeleton" role="status" aria-label="Loading invoice details">
    ...
    <div class="screen-loading-skeleton" role="status" aria-label="Loading screen content">
      ...
      <div class="screen-loading-table">
        <div :for={_ <- 1..7} class="screen-loading-table-row">
          <.skeleton_block :for={width <- ["72%", "56%", "64%", "52%", "58%", "48%", "50%", "60%", "56%", "52%"]} class="skeleton-line" width={width} />
        </div>
      </div>
    </div>
    """
