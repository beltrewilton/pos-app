defmodule PosServerWeb.UserLive do
        ...
        case Accounts.confirm_user(user) do
          {:ok, confirmed_user} ->
            {:noreply,
             ...
             |> put_flash(:info, "#{confirmed_user.name} has been confirmed.")}

          {:error, _changeset} ->
            {:noreply, put_flash(socket, :error, "The user could not be confirmed. Please try again.")}
        end

      nil ->
        {:noreply, put_flash(socket, :error, "The user could not be found.")}
    end
  end
  ...
  defp index_page(assigns) do
    ~H"""
    <main class="admin-page" aria-labelledby="users-title">
      <header class="admin-topbar">
        <a class="brand" href={~p"/admin"}><span class="brand-mark" aria-hidden="true">R</span><span>retaily</span></a>
        <div class="admin-topbar-actions">
          <button class="btn" data-variant="ghost" data-size="icon" type="button" data-theme-toggle aria-label="Toggle color theme"><span aria-hidden="true">◐</span></button>
          <a class="btn" data-variant="default" data-size="sm" href={~p"/admin/users/new"}>Create user</a>
        </div>
      </header>
      <section class="admin-content">
        <div class="admin-heading">
          <div>
            <p class="eyebrow"><span aria-hidden="true"></span> Administration</p>
            <h1 id="users-title">People</h1>
            <p>Review account status and manage workspace access.</p>
          </div>
          <a class="btn admin-create-inline" data-variant="default" href={~p"/admin/users/new"}>Create user</a>
        </div>
        <section class="card users-card" aria-labelledby="directory-title">
          <div class="card-header users-card-header">
            <div><h2 class="card-title" id="directory-title">User directory</h2><p class="card-description"><%= length(@users) %> <%= if length(@users) == 1, do: "account", else: "accounts" %></p></div>
            <div class="list-toolbar" role="search"><label class="sr-only" for="user-search">Search users</label><span aria-hidden="true">⌕</span><input class="input" id="user-search" type="search" placeholder="Search people" data-user-search autocomplete="off" /></div>
          </div>
          <div class="table-container"><table class="table" id="users-table"><caption class="sr-only">Retaily user accounts and their confirmation state.</caption><thead><tr class="table-row"><th class="table-head" scope="col">User</th><th class="table-head" scope="col">Workspace</th><th class="table-head" scope="col">Status</th><th class="table-head" scope="col"><span class="sr-only">Actions</span></th></tr></thead><tbody><tr :if={@users == []} class="table-row"><td class="table-cell users-empty" colspan="4"><strong>No accounts yet.</strong><span>Create a user to start your directory.</span><a class="btn" data-variant="outline" data-size="sm" href={~p"/admin/users/new"}>Create user</a></td></tr><tr :for={user <- @users} class="table-row" data-user-row data-search={String.downcase("#{user.name} #{user.email} #{user.tenant}")}><td class="table-cell"><div class="user-identity"><span class="user-avatar"><%= initials(user.name) %></span><span><strong><%= user.name %></strong><small><%= user.email %></small></span></div></td><td class="table-cell"><span class="workspace-name"><%= user.tenant || "—" %></span><small class="mobile-only"><%= user.email %></small></td><td class="table-cell"><span class="status-badge" data-confirmed={not is_nil(user.confirmed_at)}><i></i> <%= if user.confirmed_at, do: "Confirmed", else: "Awaiting confirmation" %></span></td><td class="table-cell users-actions"><button :if={is_nil(user.confirmed_at)} class="btn" data-variant="default" data-size="sm" type="button" phx-click="confirm_user" phx-value-id={user.id}>Confirm</button></td></tr></tbody></table></div>
        </section>
      </section>
    </main>
    """
  ...
  defp field(assigns) do
    ...
    <div class="form-field" data-invalid={@field.errors != [] || nil}>
      ...
      <input class="input" id={@field.id} name={@field.name} type={@type} value={if @type == "password", do: nil, else: @field.value} required={@required} minlength={@minlength} autocomplete={if @type == "password", do: "new-password"} aria-invalid={if @field.errors == [], do: "false", else: "true"} />
