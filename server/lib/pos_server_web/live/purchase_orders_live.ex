defmodule PosServerWeb.PurchaseOrdersLive do
  ...
  def mount(_params, session, socket) do
    with token when is_binary(token) <- session["user_token"],
      ...
      |> assign(:product_dialog, nil) |> assign(:product_form_status, "")
  ...
  def handle_event("open_line_product", %{"line_id" => line_id}, socket), do: {:noreply, socket |> assign(:product_dialog, %{line_id: line_id, product: nil}) |> assign(:product_form_status, "")}
  def handle_event("edit_line_product", %{"line_id" => line_id, "product_id" => product_id}, socket) do
    case ProductCatalog.get(socket.assigns.scope, integer(product_id)) do
      {:ok, product} -> {:noreply, socket |> assign(:product_dialog, %{line_id: line_id, product: product}) |> assign(:product_form_status, "")}
      _ -> {:noreply, put_flash(socket, :error, "Product could not be loaded.")}
    end
  end
  def handle_event("close_line_product", _, socket), do: {:noreply, socket |> assign(:product_dialog, nil) |> assign(:product_form_status, "")}
  def handle_event("save_line_product", params, socket) do
    dialog = socket.assigns.product_dialog
    attrs = product_attrs(params, socket)
    result = if dialog.product, do: ProductCatalog.update(socket.assigns.scope, dialog.product.id, attrs), else: ProductCatalog.create(socket.assigns.scope, attrs)
    case result do
      {:ok, product} ->
        socket = socket |> assign(:product_dialog, nil) |> load_orders()
        socket = if dialog.product, do: socket, else: socket |> update_line(dialog.line_id, &Map.merge(&1, %{product_id: product.id, query: product.name})) |> refresh_line_quantity(dialog.line_id)
        {:noreply, put_flash(socket, :info, if(dialog.product, do: "Product updated.", else: "Product created."))}
      {:error, :default_price_required} -> {:noreply, assign(socket, :product_form_status, "A default selling price is required.")}
      {:error, :forbidden} -> {:noreply, assign(socket, :product_form_status, "You do not have permission to save products.")}
      {:error, _} -> {:noreply, assign(socket, :product_form_status, "Product could not be saved. Check the required fields.")}
    end
  end
  def handle_event("line_quantity", %{"quantity" => quantities}, socket) when is_map(quantities) do
    {id, quantity} = Enum.at(quantities, 0, {nil, nil})
    if is_binary(id), do: {:noreply, update_line(socket, id, &Map.put(&1, :quantity, quantity || ""))}, else: {:noreply, socket}
  end
  ...
  defp product_attrs(params, socket) do
    prices = params |> Map.get("prices", %{}) |> Enum.flat_map(fn {id, amount} -> case Float.parse(to_string(amount)) do {price, ""} when price >= 0 -> [%{pricing_id: integer(id), price: price}]; _ -> [] end end)
    ...
    <.pos_layout id="purchase-orders-live" class="pos-shell invoice-view" active_page={:orders} scope={@scope} stores={@stores} store_id={@store_id}>
      <section class="catalog-panel" data-view="orders" aria-labelledby="orders-title">
        ...
        <section :if={@view in [:detail, :form]} id="purchase-order-screen" class="operations-screen purchase-order-screen" aria-labelledby="purchase-order-title"><div class="operations-fixed"><header class="topbar operations-topbar"><div class="brand-lockup"><span class="brand-mark" aria-hidden="true">E</span><div><p class="eyebrow">Operations</p><h2 id="purchase-order-title" tabindex="-1">{if @view == :form and @mode == :move, do: "Move products", else: "Purchase order"} — {active_store(@stores, @store_id)}</h2></div></div><button class="btn" type="button" data-variant="outline" data-size="sm" phx-click="back_to_orders">Back to orders</button></header></div>
          ...
          <article :if={@view == :form} class="card purchase-order-detail"><div class="card-header"><div><p class="eyebrow">Operations</p><h3 class="card-title">{if @mode == :move, do: "Move products", else: "Create purchase order"}</h3><p class="field-description">{if @mode == :move, do: "Move products from an origin store to a destination store.", else: "Add products and confirm the requested quantities."}</p></div></div><form class="form card-content" phx-submit="submit_order"><div class="order-form-grid"><div class="form-field"><label class="label" for="purchase-order-source">{if @mode == :move, do: "Origin store", else: "Source / provider"}</label><select id="purchase-order-source" class="select" name="source_id" phx-change="change_source" required><option value="" selected={@source_id == ""} disabled>Select a source</option><option :for={source <- source_options(@mode, @stores, @sources)} value={source.id} selected={to_string(source.id) == @source_id}>{source.name}</option></select></div><div class="form-field"><label class="label" for="purchase-order-destination">Destination store</label><select id="purchase-order-destination" class="select" name="destination_id" phx-change="change_destination" required><option :for={store <- @stores} value={store.id} selected={to_string(store.id) == @destination_id}>{store.name}</option></select></div></div><div id="order-lines" class="order-lines" phx-hook="PurchaseOrderLines"><div :for={line <- @lines} id={line.id} class="order-line" data-order-line data-line-id={line.id}><button class="btn" type="button" data-variant="outline" data-size="icon-sm" disabled={is_nil(line.product_id)} aria-label="Edit selected product" phx-click="edit_line_product" phx-value-line_id={line.id} phx-value-product_id={line.product_id}>✎</button><div class="product-combobox"><input class="input" type="text" value={line.query} placeholder="Search products" autocomplete="off" spellcheck="false" autocorrect="off" autocapitalize="off" role="combobox" aria-label="Product" aria-autocomplete="list" aria-expanded="false" aria-controls={"#{line.id}-products"} phx-keydown="line_picker_key" phx-key="Enter"/><button class="btn" type="button" data-variant="outline" data-size="icon-sm" aria-label="Create product" phx-click="open_line_product" phx-value-line_id={line.id}>+</button><ul id={"#{line.id}-products"} class="product-combobox-list" role="listbox" hidden><li :for={product <- @products} id={"#{line.id}-product-#{product.id}"}><button class="btn" type="button" role="option" data-variant="ghost" data-product-id={product.id} data-product-name={product.name} data-product-code={product.code || ""}>{product.name}{if product.code, do: " · #{product.code}", else: ""}</button></li></ul></div><input class="input numeric" type="text" value={line.current_quantity} readonly aria-label="Current inventory quantity"/><input class="input" type="number" min="1" name={"quantity[#{line.id}]"} value={line.quantity} aria-label="Requested quantity" phx-change="line_quantity"/><button class="btn" type="button" data-variant="ghost" data-size="sm" phx-click="remove_line" phx-value-id={line.id}>Remove</button></div></div><button class="btn" type="button" data-variant="outline" data-size="sm" phx-click="add_line">Add product</button><div class="form-actions"><button class="btn" type="submit" data-variant="default">{if @mode == :move, do: "Move products", else: "Create order"}</button></div></form></article>
        </section>
      </section>
      <dialog :if={@product_dialog} id="product-dialog" class="dialog" data-size="lg" role="dialog" aria-modal="true" aria-labelledby="product-dialog-title"><div class="dialog-content"><div class="dialog-header"><h2 id="product-dialog-title" class="dialog-title">{if @product_dialog.product, do: "Edit product", else: "Create product"}</h2><p class="dialog-description">Catalog details and pricing-list values are saved as separate records.</p></div><form id="product-form" class="form dialog-body" phx-submit="save_line_product"><div class="form-field"><label class="label" for="product-name">Name <span class="text-destructive">*</span></label><input id="product-name" name="name" class="input" value={@product_dialog.product && @product_dialog.product.name} required autofocus/></div><div class="product-form-row"><div class="form-field"><label class="label" for="product-cost">Cost</label><input id="product-cost" name="cost" class="input" type="number" min="0" step="0.01" value={@product_dialog.product && @product_dialog.product.cost || 0}/></div><fieldset class="form-fieldset product-prices-fieldset"><legend>Price lists</legend><div class="form-group"><div :for={list <- @pricing_lists} class="product-price-row"><label class="label" for={"product-price-#{list.id}"}>{list.label}</label><input id={"product-price-#{list.id}"} name={"prices[#{list.id}]"} class="input" type="number" min="0" step="0.01" placeholder="Price" value={price_for(@product_dialog.product, list.id)} required={list.id == 1}/></div></div></fieldset></div><div class="form-field"><label class="label" for="product-code">SKU</label><input id="product-code" name="code" class="input" value={@product_dialog.product && @product_dialog.product.code}/></div><div id="product-image-dropzone" class="product-image-dropzone" tabindex="0" role="button" aria-describedby="product-image-help"><img id="product-image-preview" class="product-image-preview" src={image_source(@product_dialog.product && @product_dialog.product.image_raw)} alt="Product preview" hidden={is_nil(@product_dialog.product && @product_dialog.product.image_raw)}/><label class="label" for="product-image">Image upload</label><p id="product-image-help" class="field-description">Drop an image here or choose a file (max 10 MB). It will be resized and stored as Base64.</p><input id="product-image" class="input" type="file" accept="image/*"/><input id="product-image-raw" name="image_raw" type="hidden" value={@product_dialog.product && @product_dialog.product.image_raw}/></div><p class="field-description" role="status">{@product_form_status}</p><div class="dialog-footer"><button class="btn" type="button" data-variant="outline" phx-click="close_line_product">Cancel</button><button class="btn" type="submit" data-variant="default">Save</button></div></form></div></dialog>
    </.pos_layout>
    """
  end
