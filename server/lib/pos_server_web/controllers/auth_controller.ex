defmodule PosServerWeb.AuthController do
  ...
  def login(conn, params) do
    case Authentication.login(params) do
      {:ok, token, scope} -> json(conn, %{token: token, user: %{type: scope.actor, id: scope.actor_id, login: scope.login, pic: scope.pic, tenant: scope.tenant, store_ids: scope.store_ids, scopes: scope.scopes}})
