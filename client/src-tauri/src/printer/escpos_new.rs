...
pub fn receipt(r: &Receipt) -> Vec<u8> {
 if r.payment_receipt { return payment_receipt(r); }
...
fn payment_receipt(r: &Receipt) -> Vec<u8> {
 let mut d=vec![0x1b,0x40,0x1b,0x61,0x01]; logo(&mut d,&r.logo); bold(&mut d,true); centered(&mut d,&r.company); bold(&mut d,false); centered(&mut d,&r.store); centered(&mut d,&r.store_address); centered(&mut d,&r.store_slogan); left(&mut d); optional(&mut d,&r.labels.rnc,&r.company_id); sep(&mut d); center(&mut d); bold(&mut d,true); centered(&mut d,&r.labels.payment); bold(&mut d,false); left(&mut d); pair(&mut d,&r.labels.fiscal_sequence,&r.number); pair(&mut d,&r.labels.date,&r.date_time); optional(&mut d,&r.labels.customer,&r.customer); optional(&mut d,&r.labels.cashier,&r.cashier); optional(&mut d,&r.labels.method,&r.payment_method); sep(&mut d); bold(&mut d,true); pair(&mut d,&r.labels.amount,&money(r.payment_amount,&r.language)); bold(&mut d,false); pair(&mut d,&r.labels.balance,&money(r.balance.max(0.0),&r.language)); sep(&mut d); center(&mut d); centered(&mut d,&r.labels.thank_you); centered(&mut d,&format!("{}: {}",r.labels.receipt,r.number)); d.extend_from_slice(b"\n\n\n"); d.extend_from_slice(&[0x1d,0x56,0x00]); d
}
